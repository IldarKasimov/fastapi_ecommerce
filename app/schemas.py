...
class UserCreate(BaseModel):
    ...
    role: str = Field(default="buyer", pattern="^(buyer|seller)$", description="Новая роль")


class AdminCreate(UserCreate, BaseModel):
    """Модель для создания и обновления пользователя. Используется в POST и PUT запросах."""
    role: str = Field(default="admin", pattern="^(buyer|seller|admin)$", description="Новая роль")


class UserRoleUpdate(BaseModel):
    """Модель изменения роли пользователя. Используется в PUT запросах."""
    role: str = Field(default="buyer", pattern="^(buyer|seller|admin)$", description="Изменение роли")
...
class RefreshTokenRequest(BaseModel):
    ...
    refresh_token: str = Field(..., description="строка refresh-токена")


class ReviewCreate(BaseModel):
    """Модель для создания и обновления отзыва. Используется в POST и PUT запросах."""
    product_id: int = Field(..., description="ID товара")
    comment: str | None = Field(None, min_length=10, max_length=500, description="Текст отзыва (от 10 до 500 символов)")
    grade: int = Field(..., ge=1, le=5, description="Оценка товара (от 1 до 5)")


class Review(BaseModel):
    """Модель для ответа с данными отзыва. Используется в GET-запросах."""
